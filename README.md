...
test(`Convert PDF To PNG`, async () => {
    const pngPages = await pdfToPng(pdfFilePath, { // The function accepts PDF file path or a Buffer
        ...
        enableXfa: false, // Render Xfa forms if any. Default value is true.
        ...
        outputFileMaskFunc: (pageNumber) => `page_${pageNumber}.png`, // Output filename mask function. Should return the full filename including the .png extension.
        ...
        processPagesInParallel: false, // When `true`, pages are processed concurrently. Default value is false (serial processing).

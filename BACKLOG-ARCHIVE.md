...
- [x] 🟢 ♻️ ARCH-014 Core: close `Uint8Array | ArrayBufferLike` union at getPdfFileBuffer
    - **Impl:** `getPdfFileBuffer` returns `Promise<Uint8Array>`; the ArrayBuffer branches wrap their fresh allocation in a view, and the out-of-contract fallthrough now throws `Unsupported buffer type: <shape>` instead of returning the value. `getPdfDocument(pdfFileBuffer: Uint8Array, …)` and the worker-mode copy in `pdfToPngCore` both drop their `instanceof Uint8Array ? … : new Uint8Array(…)` coercion. New `__tests__/input.shape.contract.test.ts` asserts every supported shape leaves the seam as a `Uint8Array`
    - **Rat:** the union forced two downstream modules to re-derive "what shape do we hand pdfjs", so the answer lived in three places. Closing it at the source makes `pdfInput.ts` the sole owner. Closing the RETURN type deliberately did not narrow what is ACCEPTED: `instanceof` is realm-bound and pdfjs itself took any `ArrayBuffer.isView` or numeric-`length` value, so cross-realm typed arrays/ArrayBuffers, `Uint8ClampedArray`, and byte array-likes (a JSON-round-tripped `Buffer`) are explicitly normalized rather than rejected — a review caught that the first attempt regressed all four. Only genuine non-byte-containers now throw
- [x] 🟡 ♻️ ARCH-012 Core: colocate output-folder prepare with savePNGfile
    - **Impl:** new `prepareOutputFolder(outputFolder): Promise<OutputFolderHandle>` in `src/outputWriter.ts` owns `resolve` + `mkdir` + the baseline `realpath`; `savePNGfile(name, content, folder: OutputFolderHandle)` and `new FilesystemSink(handle)` take the handle instead of two positional strings; `pdfToPngCore` no longer imports `node:fs` or `path.resolve` at all. Call order is preserved — the duplicate-name check still runs before the first output I/O, now expressed by gating on the raw `outputFolder` rather than on the resolved path. New `__tests__/output.folder.handle.test.ts` covers recursive creation, the symlinked-ancestor baseline, and a tampered-baseline write rejection
    - **Rat:** the SEC-001/002/003 threat model was split across two modules — `pdfToPngCore` created the folder and captured the realpath baseline, while `outputWriter` consumed it — and the baseline travelled as two loose strings a caller had to keep in sync. One module now owns folder lifetime, and the handle makes the pairing unforgeable
- [x] 🟢 ♻️ ARCH-016 Core: de-duplicate flat-filename rule (with ARCH-012)
    - **Impl:** new `src/flatFilename.ts` exporting `containsPathSeparator()` + `SEPARATOR_DESCRIPTION`; `pageOrchestrator.assertFlatFilename` and `savePNGfile` both call it and each keeps its own (different, tested) error message
    - **Rat:** the `sep === '\\' ? /[\\/]/ : /\//` predicate — load-bearing for SEC-001, since rejecting separators is what closes the TOCTOU window on intermediate directory components — was duplicated verbatim in two modules, so a future fix could land in one and silently miss the other

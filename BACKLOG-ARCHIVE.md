...
- [x] 🟢 ♻️ ARCH-014 Core: close `Uint8Array | ArrayBufferLike` union at getPdfFileBuffer
    ...
    - **Rat:** the union forced two downstream modules to re-derive "what shape do we hand pdfjs", so the answer lived in three places. Closing it at the source makes `pdfInput.ts` the sole owner. Note the deliberate behaviour change for JavaScript callers passing an unsupported shape: previously pdfjs reported `The PDF file is empty, i.e. its size is zero bytes.`, which described the wrong problem
